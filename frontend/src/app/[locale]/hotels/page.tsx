...
import {
  Star, MapPin, Search, Building2, ChevronDown, SlidersHorizontal, X
} from 'lucide-react';
...
const fetchLocations = async () => {
  const res: any = await api.get('/hotels/locations');
      ...
      <div className="h-72 md:h-96 rounded-3xl overflow-hidden relative mb-10 shadow-2xl bg-slate-900 group">
        ...
        <div className="absolute bottom-8 left-8 right-8 z-10 max-w-2xl text-white">
          ...
          <h1 className="text-3xl md:text-5xl font-extrabold mb-3 tracking-tight leading-tight">
            Khách Sạn & Resort Đẳng Cấp
          </h1>
          <p className="text-blue-100/90 text-sm md:text-base leading-relaxed">
            Đặt phòng nghỉ dưỡng cao cấp, đầy đủ tiện nghi với mức giá tốt nhất từ Travel.
          </p>
        </div>
      </div>
              ...
              <div className="flex gap-2 flex-wrap">
                {[0, 5, 4, 3].map(s => (
                  <button key={s} onClick={() => setStarFilter(s)}
                    ...
                    {s === 0 ? t('allStars', { fallback: 'Tất cả' }) : (
                      <><Star className="w-3 h-3 fill-amber-400 text-amber-400" />{s} Sao</>
                    )}
                  </button>
                ))}
              </div>
