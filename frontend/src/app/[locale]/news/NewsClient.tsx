...
export default function NewsClient({ initialNews, newsBanner }: Props) {
  const locale = useParams().locale as string;
  const [selectedCategory, setSelectedCategory] = useState('');

  const { data: newsList = initialNews, isLoading } = useQuery({
    queryKey: ['news', selectedCategory],
    queryFn: () => fetchNews(selectedCategory),
    placeholderData: selectedCategory ? keepPreviousData : initialNews as any,
    enabled: selectedCategory !== '',
    staleTime: 5 * 60 * 1000,
  });

  const displayNews = selectedCategory ? newsList : (newsList.length ? newsList : initialNews);
  ...
  return (
    <div className="container mx-auto px-4 py-12 mt-20">
      ...
      <div className="h-64 md:h-80 rounded-3xl overflow-hidden relative mb-10 shadow-2xl bg-slate-900 group">
        <img src={newsBanner} alt="News Banner" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
            ...
            <Link href={`/${locale}/news/${featured.slug}`}
              ...
              <div className="flex flex-col md:flex-row">
                ...
                <div className="md:w-1/2 p-8 flex flex-col justify-center">
                  ...
                  <p className="text-slate-500 leading-relaxed line-clamp-3 mb-6">{getI18nText(featured.excerpt, locale)}</p>
