...
const slides = [
  {
    ...
    subtitle: {
      ...
      ja: 'ハ노イ36古街の歴史ある美しさ、文化、絶品グルメをご体感ください'
    },
  ...
  },
  {
    ...
    ctaText: { vi: 'Đặt Khách Sạn', en: 'Book Hotels', zh: '预订酒店', ko: '호텔 예약', ja: 'ホテル를 예약' },
    ctaLink: '/hotels',
  },
  {
...
async function seedSettings() {
  try {
    ...
    await db.collection('settings').updateOne(
      { key: 'hero_banners' },
      { $set: { group: 'homepage', value: slides } },
      { upsert: true }
    );

    const siteInfoObj = await db.collection('settings').findOne({ key: 'site_info' });
    const currentSiteInfo = siteInfoObj?.value || { siteName: 'Travel Booking', hotline: '1800 646 888', email: 'support@travel.com' };
    currentSiteInfo.address = address;

    await db.collection('settings').updateOne(
      { key: 'site_info' },
      { $set: { group: 'general', value: currentSiteInfo } },
      { upsert: true }
    );
    
    console.log('Successfully seeded multi-language settings into MongoDB!');
    mongoose.disconnect();
  } catch (err) {
